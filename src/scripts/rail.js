(() => {
  ...
  const timing = { lightMin:6000, lightMax:26000, firstLightMax:18000, lightDurationMin:2000, lightDurationMax:3900, planeDuration:54000, planeMin:55000, planeMax:100000 };
  ...
  function shimmer(light,first=false) {
    later(()=>{
      if(!running)return;
      animate(light,[{opacity:0},{opacity:random(.28,.62),offset:.46},{opacity:.12,offset:.73},{opacity:0}],{duration:random(timing.lightDurationMin,timing.lightDurationMax),easing:'ease-in-out'});
      shimmer(light);
    },first?random(500,timing.firstLightMax):random(timing.lightMin,timing.lightMax));
  }
  function fly(first=false) {
    later(()=>{
      if(!running)return;
      animate(aircraft,[{transform:'translate(920px, 92px)',opacity:0},{transform:'translate(970px, 94px)',opacity:.8,offset:.05},{transform:'translate(1860px, 139px)',opacity:.8,offset:.94},{transform:'translate(1920px, 142px)',opacity:0}],{duration:timing.planeDuration,easing:'linear'});
      later(()=>fly(),timing.planeDuration);
    },first?random(8000,16000):random(timing.planeMin,timing.planeMax));
  }
