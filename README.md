...
### v0.6 static sky, direct scene interaction

The owner requested restoring the original photograph/static clouds and removing all three hero controls. The generated sky asset/layer, inspector and debug/control code are removed from the current source. v0.5 remains available unchanged. All 29 contours, 180 light positions, signal/aircraft/scanline behavior and following translucent cards remain. Tap objects directly; use Tab to enter the scene and arrow keys to move between objects. Escape, outside interaction or repeated touch dismisses details. OS reduced-motion and offscreen suspension continue to apply. Current release checks use `rail-static-sky-test.cjs`, `rail-motion-test.cjs`, `rail-visual-motion.cjs` and `rail-preservation.cjs`; historical control/cloud tests belong to their corresponding releases.
