...
- Owner's v0.5 request expands the scene inventory and explicitly removes the tooltip close cross and lower disclaimer. Keep descriptions illustrative and free of invented specifications. Hover cards are translucent, pointer-transparent and cursor-following; touch/keyboard use Explore scene, Escape and outside dismissal. The separate sky plate moves only inside the source-coordinate skyline mask. Require `rail-scene-expansion-test.cjs` for scene changes, including rendered cloud isolation, protected silhouettes and touch/keyboard dismissal.
- Owner's v0.6 correction supersedes v0.5 sky/control behavior: restore the original static-sky photograph; remove Explore scene, Pause motion and Light debug, including their obsolete runtime and styles. Preserve all contours, hover/touch cards and other ambient effects. Keyboard access uses direct scene focus and arrow keys; OS reduced-motion/offscreen suspension remains. Run `rail-static-sky-test.cjs`, the adapted motion/rendered-motion tests and preservation checks. Cloud/control history remains available in v0.5.
