...
function heroScene(){
  ...
  const lights=scene.lights.map(([cx,cy,r])=>`<g class="scene-light"><circle class="scene-light-halo" cx="${cx}" cy="${cy}" r="${r*3.6}"/><circle class="scene-light-core" cx="${cx}" cy="${cy}" r="${r}"/></g>`).join('');
