...
function heroScene(){
  ...
  const lights=scene.lights.map(([cx,cy,r,group])=>`<g class="scene-light" data-energy="${scene.lightGroups[group].energy}" style="--light-energy:${scene.lightGroups[group].energy}"><circle class="scene-light-halo" cx="${cx}" cy="${cy}" r="${r*3.6}"/><circle class="scene-light-core" cx="${cx}" cy="${cy}" r="${r}"/></g>`).join('');
